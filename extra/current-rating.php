...
$q = mysql_query("SELECT * FROM `ratings` WHERE `id`='$productIdByPermalink'");
$n = mysql_num_rows($q);
if($n==1)
	$v = $lang_array['vote'];
else
...
$q = mysql_query("SELECT * FROM ratings WHERE `id`='$productIdToPermalink' AND `ip`='$ip'");
...
<div class="rating">
	...
	<script>
		...
		$(".star").click(function()
		{
			var the_id = '<?php echo $productIdByPermalink?>';
			var rating = $(this).attr("id");
			for(i=(rating-1); i>=0; i--)
			{
				$(".rating .star:eq("+i+")").addClass("active");
			}
			var data = 'rating='+rating+'&id=<?php echo $productIdByPermalink; ?>&page=product';
			$.ajax
			({
				type: "POST", 
				data: data,
